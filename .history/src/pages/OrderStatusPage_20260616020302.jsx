...
//   const [currentStatus, setCurrentStatus] = useState(0); // 0=waiting_payment, 1=payment_success, 2=kitchen, 3=cooking, 4=ready
  const [loading, setLoading] = useState(false);
  const [role, setRole] = useState("customer"); // customer, kasir, kitchen

  // Simulasi cek role (nanti dari login/token)
  useEffect(() => {
    // Cek apakah user adalah kasir atau kitchen
...
const fetchLatestStatus = async () => {
  try {
    ...
    const newStatus = statusMap[statusData.status] || 0;
    ...
    <div className="min-h-screen bg-gray-100 pb-6 max-w-md mx-auto">
      ...
      <div className="mx-4 mt-5 mb-6">
        ...
        >
          ...
          {loading ? "Memperbarui..." : 
            currentStatus >= statuses.length - 1 ? "Pesanan Selesai" :
            canUpdate() ? `Update ke: ${statuses[currentStatus + 1]?.name}` : "Menunggu Update dari Pihak Restoran"}
        </button>
      </div>

      {/* Role Info (untuk development) */}
      <div className="text-center text-xs text-gray-400 pb-4">
        Role saat ini: {role} | Status: {currentStatus + 1}/{statuses.length}
      </div>
    </div>
  );
}