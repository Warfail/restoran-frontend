...
export default function MenuManagementPage() {
  ...
  const toggleStatus = async (menuId, currentStatus) => {
    try {
      const newStatus = currentStatus === "AVAILABLE" ? "DISABLED" : "AVAILABLE";
      await api.updateMenu(menuId, { isAvailable: newStatus === "AVAILABLE" });
      await fetchMenus();
    } catch (error) {
                      ...
                      <td className="px-5 py-3">
                        <div className="flex gap-2">
                          <button className="w-8 h-8 rounded-md border border-gray-200 flex items-center justify-center hover:bg-gray-50">
                            <button onClick={() => openUpdateModal(menu)} className="text-gray-400 hover:text-gray-600">
