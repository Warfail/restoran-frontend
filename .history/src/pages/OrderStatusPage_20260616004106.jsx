...
import { Check, UtensilsCrossed, Flame, ChefHat, Info, RefreshCw } from "lucide-react";
...
export default function OrderStatusPage() {
  ...
  const [currentStatus, setCurrentStatus] = useState(0); // 0=payment, 1=kitchen, 2=cooking, 3=ready
  const [loading, setLoading] = useState(false);

  // Simulasi check status dari backend
  useEffect(() => {
    // Cek status awal (misal dari API)
    // Di sini simulasi status sudah di payment
    setCurrentStatus(0);
  }, []);

  const statuses = [
    { id: 0, name: "Pembayaran Berhasil", icon: Check, description: "Pembayaran digital Anda telah terverifikasi." },
    { id: 1, name: "Pesanan Diterima Dapur", icon: UtensilsCrossed, description: "Pesanan Anda sedang antre untuk dimasak." },
    { id: 2, name: "Sedang Dimasak", icon: Flame, description: "Menunggu giliran penggorengan." },
    { id: 3, name: "Siap Disajikan", icon: ChefHat, description: "Sajian hangat akan segera diantar." },
  ];

  const handleRefresh = () => {
    setLoading(true);
    // Simulasi update status dari backend
    setTimeout(() => {
      setCurrentStatus(prev => Math.min(prev + 1, 3));
      setLoading(false);
    }, 1000);
  };
    ...
    <div className="min-h-screen bg-gray-100 pb-6 max-w-md mx-auto">
      ...
      <div className="mx-4 mt-4 bg-white rounded-xl shadow-sm p-4">
        ...
        <div className="flex justify-between items-center">
          <span className="text-gray-900 font-bold text-sm">Total Pesanan: {totalItems} Item</span>
          <span className="text-amber-500 font-bold text-sm">Rp {totalAmount.toLocaleString()}</span>
        </div>
      </div>
      ...
      <div className="mx-4 mt-5 mb-6">
        <button
          ...
          disabled={loading}
          className="w-full bg-red-500 hover:bg-red-600 text-white font-semibold text-base py-4 rounded-xl flex items-center justify-center gap-2 transition disabled:opacity-50"
        >
