...
export default function CashierDashboard() {
  ...
  const confirmOrder = async (orderId) => {
    try {
      const response = await api.updateOrderStatus(orderId, "confirmed");
      if (response.success) {
        await fetchOrders();
  ...
  const processPayment = async () => {
    ...
    const response = await api.processCashPayment(selectedOrder.orderId, amount);
  if (response.success) {
    await fetchOrders();
    ...
    try {
      // Ganti api.processPayment menjadi api.processCashPayment
                      ...
                      <td className="px-6 py-4 text-center">
                        <div className="flex items-center justify-center gap-2">
                          ...
                          {order.status === "pending" && (
                            <button onClick={() => confirmOrder(order.orderId)} className="p-1.5 rounded-lg bg-blue-50 hover:bg-blue-100" title="Konfirmasi">
